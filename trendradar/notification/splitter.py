...
def split_content_into_batches(
    ...
    report_type: str = "Piyasa Analiz Raporu",
    show_new_section: bool = True,
) -> List[str]:
    """分批处理消息内容，确保词组标题+至少第一条新闻的完整性（支持热榜+RSS合并+AI分析+Bağımsız Bölüm）

    热榜统计与RSS统计并列显示，热榜新增与RSS新增并列显示。
    region_order 控制各区域的显示顺序。
    AI分析内容根据 region_order 中的位置显示。
    Bağımsız Bölüm根据 region_order 中的位置显示。
    ...
    Args:
        ...
        standalone_data: Bağımsız Bölüm数据（可选），包含 platforms 和 rss_feeds 列表
    ...
    total_news_line = f"{b_s}Toplam haber:{b_e} {total_titles}"
    if total_new > 0:
        total_news_line += f" (yeni {new_count} + {rss_new_count})"
    base_header += f"{total_news_line}\n"

    # 2. 热榜
    hotlist_info = f"{b_s}Hot-list:{b_e} {total_hotlist_count}/{hotlist_total}"
    if platform_total > 0:
        hotlist_info += f" (platform {platform_success}/{platform_total})"
    base_header += f"{hotlist_info}\n"

    # 3. RSS
    if rss_source_total > 0:
        rss_info = f"{b_s}RSS:{b_e} {rss_matched}/{rss_total_items} (kaynak {rss_source_success}/{rss_source_total})"
        base_header += f"{rss_info}\n"

    # 4. Bağımsız Bölüm（仅在有数据时显示）
    if standalone_data:
        ...
        if sa_total > 0:
            ...
            if sa_platform_count > 0:
                sa_parts.append(f"Hot-list {sa_platform_count}")
            if sa_rss_count > 0:
                sa_parts.append(f"RSS {sa_rss_count}")
            base_header += f"{b_s}Bağımsız:{b_e} {sa_total} ({' + '.join(sa_parts)})\n"
    ...
    if ai_has_data:
        ...
        if ai_mode_val and ai_mode_val != mode:
            mode_map = {"daily": "Günlük Özet", "current": "Anlık Liste", "incremental": "Artımlı Analiz"}
            mode_suffix = f" [{mode_map.get(ai_mode_val, ai_mode_val)}]"

        base_header += f"{b_s}AI Analizi:{b_e} {ai_display}{mode_suffix}\n"

    # === 空行分隔 ===
    base_header += "\n"

    # === 下半部分：元信息 ===
    base_header += f"{b_s}Tip:{b_e} {report_type}\n"
    base_header += f"{b_s}Zaman:{b_e} {now.strftime('%Y-%m-%d %H:%M:%S')}\n"

    top_words = report_data.get("stats", [])[:3]
    if top_words:
        topics = " | ".join(f"{s['word']}({s['count']})" for s in top_words)
        base_header += f"{b_s}En sıcak konular:{b_e} {topics}\n"
    ...
    if format_type in ("wework", "bark"):
        base_footer = f"\n\n\n> Güncelleme:{now.strftime('%Y-%m-%d %H:%M:%S')}"
        if update_info:
            base_footer += f"\n> TrendRadar yeni sürüm buldu:**{update_info['remote_version']}**, mevcut:**{update_info['current_version']}**"
    elif format_type == "telegram":
        base_footer = f"\n\nGüncelleme:{now.strftime('%Y-%m-%d %H:%M:%S')}"
        if update_info:
            base_footer += f"\nTrendRadar yeni sürüm buldu:{update_info['remote_version']}, mevcut:{update_info['current_version']}"
        ...
        base_footer = f"\n\n<font color='grey'>Güncelleme:{now.strftime('%Y-%m-%d %H:%M:%S')}</font>"
        if update_info:
            base_footer += f"\n<font color='grey'>TrendRadar yeni sürüm buldu:{update_info['remote_version']}, mevcut:{update_info['current_version']}</font>"
    elif format_type == "dingtalk":
        base_footer = f"\n\n> Güncelleme:{now.strftime('%Y-%m-%d %H:%M:%S')}"
        if update_info:
            base_footer += f"\n> TrendRadar yeni sürüm buldu:**{update_info['remote_version']}**, mevcut:**{update_info['current_version']}**"
    elif format_type == "slack":
        base_footer = f"\n\n_Güncelleme:{now.strftime('%Y-%m-%d %H:%M:%S')}_"
        if update_info:
            base_footer += f"\n_TrendRadar yeni sürüm buldu:*{update_info['remote_version']}*, mevcut:*{update_info['current_version']}_"

    # 根据 display_mode 选择统计标题
    stats_title = "Anahtar Kelime İstatistiği" if display_mode == "keyword" else "Haber İstatistiği"
    stats_header = ""
    if report_data["stats"]:
        if format_type in ("wework", "bark"):
            stats_header = f"📊 **{stats_title}** (toplam {total_hotlist_count})\n\n"
        elif format_type == "telegram":
            stats_header = f"📊 {stats_title} (toplam {total_hotlist_count})\n\n"
        elif format_type == "ntfy":
            stats_header = f"📊 **{stats_title}** (toplam {total_hotlist_count})\n\n"
    ...
    if (
        ...
        and not standalone_data  # 有Bağımsız Bölüm数据时也不返回
    ):
        if mode == "incremental":
            mode_text = "Artımlı modda yeni eşleşen başlık yok"
        elif mode == "current":
            mode_text = "Anlık liste modunda eşleşen başlık yok"
        else:
            mode_text = "Eşleşen başlık yok"
    ...
    def process_new_titles_section(current_batch, current_batch_has_content, batches, add_separator=True):
        ...
        if add_separator and current_batch_has_content:
            # 需要添加分割线
            if format_type in ("wework", "bark"):
                new_header = f"\n\n\n\n🆕 **Bu turdaki yeni haberler** (toplam {report_data['total_new_count']})\n\n"
            elif format_type == "telegram":
                new_header = (
                    f"\n\n🆕 Bu turdaki yeni haberler (toplam {report_data['total_new_count']})\n\n"
                )
            ...
            # 不需要分割线（第一个区域）
            if format_type in ("wework", "bark"):
                new_header = f"🆕 **Bu turdaki yeni haberler** (toplam {report_data['total_new_count']})\n\n"
            elif format_type == "telegram":
                new_header = f"🆕 Bu turdaki yeni haberler (toplam {report_data['total_new_count']})\n\n"
            elif format_type == "ntfy":
                new_header = f"🆕 **Bu turdaki yeni haberler** (toplam {report_data['total_new_count']})\n\n"
    ...
    if report_data["failed_ids"]:
        failed_header = ""
        if format_type == "wework":
            failed_header = f"\n\n\n\n⚠️ **Veri çekilemeyen platformlar:**\n\n"
        elif format_type == "telegram":
            failed_header = f"\n\n⚠️ Veri çekilemeyen platformlar:\n\n"
        elif format_type == "ntfy":
            failed_header = f"\n\n⚠️ **Veri çekilemeyen platformlar:**\n\n"
...
def _process_rss_stats_section(
    ...
    if add_separator and current_batch_has_content:
        # 需要添加分割线
        if format_type == "feishu":
            rss_header = f"\n{feishu_separator}\n\n📰 **RSS Aboneliği** (toplam {total_items})\n\n"
        elif format_type == "dingtalk":
            rss_header = f"\n---\n\n📰 **RSS Aboneliği** (toplam {total_items})\n\n"
        elif format_type in ("wework", "bark"):
            rss_header = f"\n\n\n\n📰 **RSS Aboneliği** (toplam {total_items})\n\n"
        ...
        # 不需要分割线（第一个区域）
        if format_type == "feishu":
            rss_header = f"📰 **RSS Aboneliği** (toplam {total_items})\n\n"
        elif format_type == "dingtalk":
            rss_header = f"📰 **RSS Aboneliği** (toplam {total_items})\n\n"
        elif format_type == "telegram":
            rss_header = f"📰 RSS Aboneliği (toplam {total_items})\n\n"
...
def _process_rss_new_titles_section(
    ...
    if add_separator and current_batch_has_content:
        # 需要添加分割线
        if format_type in ("wework", "bark"):
            new_header = f"\n\n\n\n🆕 **RSS yeni eklenenler** (toplam {total_items})\n\n"
        elif format_type == "telegram":
            new_header = f"\n\n🆕 RSS yeni eklenenler (toplam {total_items})\n\n"
        elif format_type == "ntfy":
            new_header = f"\n\n🆕 **RSS yeni eklenenler** (toplam {total_items})\n\n"
        ...
        # 不需要分割线（第一个区域）
        if format_type in ("wework", "bark"):
            new_header = f"🆕 **RSS yeni eklenenler** (toplam {total_items})\n\n"
        elif format_type == "telegram":
            new_header = f"🆕 RSS yeni eklenenler (toplam {total_items})\n\n"
        elif format_type == "ntfy":
            new_header = f"🆕 **RSS yeni eklenenler** (toplam {total_items})\n\n"
...
def _process_standalone_section(
    ...
    Bağımsız Bölüm显示指定平台的完整热榜或 RSS 源内容，不受关键词过滤影响。
    ...
    # Bağımsız Bölüm标题（根据 add_separator 决定是否添加前置分割线）
    section_header = ""
    if add_separator and current_batch_has_content:
        # 需要添加分割线
        if format_type == "feishu":
            section_header = f"\n{feishu_separator}\n\n📋 **Bağımsız Bölüm** (toplam {total_items})\n\n"
        elif format_type == "dingtalk":
            section_header = f"\n---\n\n📋 **Bağımsız Bölüm** (toplam {total_items})\n\n"
        elif format_type in ("wework", "bark"):
            section_header = f"\n\n\n\n📋 **Bağımsız Bölüm** (toplam {total_items})\n\n"
        ...
        # 不需要分割线（第一个区域）
        if format_type == "feishu":
            section_header = f"📋 **Bağımsız Bölüm** (toplam {total_items})\n\n"
        elif format_type == "dingtalk":
            section_header = f"📋 **Bağımsız Bölüm** (toplam {total_items})\n\n"
        elif format_type == "telegram":
            section_header = f"📋 Bağımsız Bölüm (toplam {total_items})\n\n"
...
def _format_standalone_platform_item(item: Dict, index: int, format_type: str, rank_threshold: int = 10) -> str:
    """格式化Bağımsız Bölüm的热榜条目（复用热点词汇统计区样式）
    ...
    count_display = f"({count} kez)" if count > 1 else ""
